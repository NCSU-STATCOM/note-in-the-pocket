...
source(here("warpDLM-reproducible-code-main/Code/helper_functions.R"))
...
# run through zoho_data_wrangling to get dat
dat <- matrix(c(41,   50,
                28,   49,
                40,   58,
                38,   60,
...
boys_tops10_np <- SUTSE_mcmc_dlm(dat, init_mod, update_mod = updatemod_invWish_dlm, transformation = "np",
                                  nsave=10000, nburn=5000, nskip=1, nfc=1, particle = T)


save(boys_tops10_np, file = here("zoho_forecasting/boys_tops10_np.RData"))
